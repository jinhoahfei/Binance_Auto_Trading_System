...
| LOWER_TOUCH_WATCH | 하단 터치 감시 상태 | 현재가의 하단 BB 접촉을 감시하는 상태 |
...
| ---- | ----------------- | ------------------- | ------------------------- | ------------------------------------- | ----------------- |
| G-01 | 시작 | 로직 시작 | None | LOGIC_ENABLED 초기화 | LOWER_TOUCH_WATCH |
| G-02 | LOWER_TOUCH_WATCH | 하단 BB 접촉 | STM 표기: 현재 가격 <= 하단 볼린저밴드 | 1) ACTIVATE_TRADE_MANAGEMENT Event 발생 | TRADE_MANAGEMENT |
| G-03 | TRADE_MANAGEMENT | TRADE_MANAGEMENT 완료 | 포지션과 감시 하위 상태 머신이 모두 종료됨 | 1) position_owner = None, 2) 하단 터치 감시 | LOWER_TOUCH_WATCH |
| G-04 | 임의의 무포지션 상태 | 매매 중지 | owner  NONE | 로직을 종료한다. | LOGIC_TERMINATED |
| G-05 | 임의의 포지션 보유 상태 | 매매 중지[포지션 보유] | owner in {CASE_B, CASE_C} | 1) 포지션 강제 매도, 2) 주문 결과 확인, 3) 로직 중지 | LOGIC_TERMINATED |
...
##### Case B 청산 판정 권장 우선순위  
  
1. 비상 손절  
2. 확정 30분봉 일반 손절  
3. ==%B >= 0.60== 진입 시 강한 반등 여부 판정  
4. 일반 익절  
5. 시간 청산  
6. 상단 BB 인계(현재 미구현)
  
동일 평가 주기에서 여러 조건이 동시에 참일 때의 최종 우선순위는 확인이 필요하다.  
...
1. ==%B >= 0.10== 익절권 진입  
2. ==ema_slope <= -0.55== 3분 손절  
3. 60분 시간 청산  
  
익절권 진입 후:  
  
1. 실시간 ==%B < 0.10== fallback  
2. 1분봉 마감 기준 30분봉 EMA slope 상승·하락 비교  
3. 60분 시간 청산  
...
- ==BBW < 0.02==는 매수 시점 값이 아니라 **하단 BB 터치봉 값**이다.  
- signal은 하단 터치 뒤 최초로 조건을 만족한 확정 30분봉 하나만 인정한다.  
- 3시간은 signal 확정 시각부터 계산한다.  
- 상태도의 ==GO_TO_WAIT_SIGNAL==은 Case C 청산 후 조건부 Case B 재개용 내부 EVENT로 보인다.  
...
- ==entry_pct_b = timer_base_pct_b + 0.06==으로 계산한다.  
- 더 낮은 가격이 발생한 경우 C-06의 flush 갱신을 C-07 타이머 갱신보다 먼저 처리한다.  
- 한 30분봉에서 Case C setup은 한 번만 인정한다.  
- ==realtime_pct_b >= 0.25== 회복 전까지는 청산 후 Case C 재진입을 허용하지 않는 것이 기술서 기준이다.  
