...
| G-03 | TRADE_MANAGEMENT | 새 30분봉 하단 BB 접촉 | `position_owner == None` && `pending_order_id == None` && `current_30m_candle_id != touch_candle_id` && `current_30m_low <= lower_band` && (`case_c_consumed_for_event == False` OR `case_c_recovery_confirmed == True`) | 1) 기존 하단 터치 이벤트를 종료한다. 2) 현재 30분봉을 `touch_candle`로 하여 새 `lower_event_id`를 생성한다. 3) G-02의 event-local Context 플래그를 재초기화한다. 4) ACTIVATE_TRADE_MANAGEMENT EVENT를 queue에 넣는다. | TRADE_MANAGEMENT의 세 Region initial 상태로 재진입 |
...
| B-02 | B_WAIT_TOUCH | ACTIVATE_TRADE_MANAGEMENT | `touch_candle_low > lower_band_at_touch` OR `touch_candle_bbw >= 0.02` | 1) `case_b_enabled = False`를 적용한다. 2) Case B runtime Context를 초기화한다. | CASE_B_FINAL_STATE |
