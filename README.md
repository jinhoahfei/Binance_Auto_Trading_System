...
Binance Spot `ETHUSDT` long-only 자동 거래 시스템입니다. 현재 Phase 9의 실제
Spot Testnet REST/WebSocket adapter와 opt-in 검증 suite가 구현되어 있지만,
credential을 사용한 Testnet 검증은 아직 완료되지 않았습니다. 기본 실행은
fail-closed이며 `live` 주문은 Phase 13의 별도 승인 전까지 잠겨 있습니다.
...
- `BINANCE_TESTNET_MAX_NOTIONAL`은 decision price와 준비 수량으로 계산하는 로컬 사전
  상한입니다. MARKET 체결 금액의 하드 상한은 아니며, 실제 체결 금액과 거래소의
  평균/reference 가격 기반 filter 판정이 가격 변동과 slippage를 포함해 최종 권위입니다.
...
| `BINANCE_TESTNET_MAX_NOTIONAL` | 주문 opt-in 시 필수인 decision-price 기준 양수 quote 사전 상한 |
...
open order와 recent execution의 normalized contract만 실제 Testnet에서 확인합니다.
주문 mutation은 수행하지 않습니다.
...
발생시킬 수 있습니다. 이 상한은 decision-price 사전 검사이므로 주문 직전 잔액과
Testnet 시세를 다시 확인해야 합니다.
...
현재 repository에서는 production adapter와 로컬 검증이 완료됐지만 사용자 Testnet
credential이 없어 authenticated read-only, capped BUY/force-sell lifecycle과
open-order/position 실제 restart scenario는 아직 실행하지 않았습니다. 따라서 Phase 9
전체 완료로 표기하지 않습니다.

구현 범위와 미완료 실제 검증 상태는
