...
## macOS live read-only profile (Session 7)

Live와 Testnet은 서로 다른 Keychain service와 저장소를 사용한다. 앱을 정상 종료한 뒤 아래 명령을
실행하고 `LIVE`를 입력한다. API key를 동일하게 두 번 입력한 뒤 API secret을 동일하게 두 번 입력한다.
입력 중 문자는 표시되지 않는다. 키 원문을 채팅·환경변수·일반 설정 파일에 넣지 않는다.

```sh
/bin/sh /Users/oscar/Desktop/Binance_Auto/scripts/configure_live_keychain.sh set
```

이 명령은 `com.binance-auto.trader.live`의 `api-key`/`api-secret`과 native `LIVE_READ_ONLY` profile을
설정한다. Binance 본계정의 읽기 권한·IP 설정을 확인하고 withdrawal 권한은 비활성화한다.
앱을 다시 시작하면 실계좌·주문 비활성 표시가 나타나야 한다. 기본 profile 부재 또는 `disable`은
기존 Testnet 경로이며 live credential/이력을 fallback하지 않는다.

Signed GET만 사전 검사하려면 repository root에서 실행한다.
...
Native 앱을 정상 종료한 뒤 동일 live 저장소의 실제 graph와 owner lock을 검증할 수 있다.

```sh
PYTHONPATH=backend/src backend/.venv/bin/python scripts/live_runtime_readiness.py --confirm-live LIVE
```

이 도구는 실제 signed REST/WS로 READY·Position/pending/unknown·거래소 미체결 상태와 정상 종료를
검사한다. 주문은 비활성이고 native와 동시 실행은 거부한다. `runtime_readiness=PASS`도 수수료·잔액
조건이나 별도 실제 주문 승인을 대체하지 않는다.
