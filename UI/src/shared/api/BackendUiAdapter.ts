...
export function validate_connection_descriptor(
    ...
    // 연결 정보가 객체인지 확인한 뒤 포트·세션·인증 token의 형식을 검증한다.
    ...
    // 지원하는 schema와 연결 필드가 모두 맞을 때만 descriptor를 수락한다.
    if (!is_valid_port
        || !is_valid_session
...
function validate_split_ratio_command_result(
    ...
    // 공통 명령 version과 매수·매도 비율의 문자열 범위를 검증한다.
...
function validate_trading_command_result(value: unknown): BackendTradingCommandResult {
    ...
    // 실행 중인 상태는 반드시 유효한 session에 속해야 한다.
...
function validate_shutdown_receipt(value: unknown): BackendShutdownReceipt {
    // 종료 수락 영수증의 필드와 accepted 상태를 검증한다.
...
function validate_sidecar_exit_receipt(value: unknown): void {
    // 네이티브 영수증이 종료 완료와 정상 종료 코드를 함께 보장하는지 확인한다.
    const receipt = require_exact_record(
        value,
        ['exited', 'code'],
...
function validate_shutdown_preparation(value: unknown): BackendShutdownPreparation {
    // 선택적 잔고 대조 정보를 포함해 응답의 정확한 필드 집합을 정한다.
    ...
    // 잔고 대조와 종료 작업 식별자·단계·오류 정보를 함께 검증한다.
...
/**
 * 함수 이름: validate_shutdown_state()
 * 기능: 종료 상태 응답의 session·lifecycle·version 계약을 검사한다.
 * 인자: value -> 응답 payload, session_id -> 현재 backend session
 * 반환값: 검증한 종료 상태; 계약이 맞지 않으면 예외
 * 작성 날짜: 2026/09/17
 */
...
function classify_native_sidecar_wait_failure(
    ...
    // 알려진 네이티브 timeout 코드만 대기 초과로 분류하고 나머지는 비정상 종료로 취급한다.
...
function parse_local_date_epoch(value: unknown): number {
    ...
    // 실제 달력 날짜로 왕복 변환해 존재하지 않는 날짜의 자동 보정을 거부한다.
...
function validate_csv_receipt(value: unknown): CsvExportReceipt {
    // CSV 영수증의 필드와 파일 경로·내보낸 행 수를 검증한다.
    const receipt = require_exact_record(
        value,
...
export class BackendUiAdapter implements UiCommandPort {
    ...
    constructor(
        ...
        // 신뢰할 수 있는 연결 정보와 각 작업의 제한 시간을 준비한다.
        ...
        // 유효하지 않은 시간 설정은 실행 의존성을 보관하기 전에 거부한다.
        ...
        // 검증한 세션 정보와 주입된 I/O 구현을 adapter 수명에 보관한다.
     ...
     * 인자: signal -> 선택적 요청 취소 신호, generation -> 응답을 수락할 연결 세대
    ...
    start_live_events(
        ...
        // snapshot의 세션과 adapter의 활성 수명을 확인한다.
        if (snapshot.session_id !== this.session_id) {
            throw new BackendAdapterError(
                'SESSION_MISMATCH',
        ...
        // snapshot을 기준으로 sequence와 명령 상태를 동기화한다.
        ...
        // 중복 이벤트 기록을 초기화한 뒤 환경 관찰과 이벤트 수신을 시작한다.
     ...
     */
    stop(): void {
        // 이미 폐기된 adapter는 중복 정리하지 않는다.
        if (this.#is_stopped && this.#session_token === null) {
            return;
        }

        // 복구 예약과 환경 listener를 먼저 정리한다.
        ...
        // 연결 세대를 바꾸고 socket·요청·중복 기록을 폐기한다.
    ...
    async load_trade_history(
        ...
        // 기간·방향 필터와 취소 신호를 상세 조회에 전달하고 응답의 query 일치를 검증한다.
     ...
     * 인자: liquidation_confirmed -> 복구 포지션 청산을 사용자가 확인했는지 여부
     * 반환값: backend typed 결과 Promise
     * 작성 날짜: 2026/08/21
     */
    ...
    /**
     * 함수 이름: run_shutdown()
     * 기능: 중복 종료 요청을 한 Promise로 합치고 연결 복구를 정리한 뒤 정상 종료를 기다린다.
     * 인자: _recovery -> 기존 호출 형태의 복구 구분 인자, liquidation_confirmed -> 청산 동의 여부
     * 반환값: backend 종료 완료 Promise
     * 작성 날짜: 2026/09/17
     */
    private async run_shutdown(_recovery: boolean, liquidation_confirmed = false): Promise<void> {
        // 동시에 들어온 종료 요청은 하나의 진행 중 Promise를 공유한다.
        if (this.#shutdown_task !== null) return this.#shutdown_task;

        // 종료 중에는 이전 연결의 복구 작업과 이벤트 수신을 중단한다.
        ...
        /**
         * 함수 이름: operation()
         * 기능: 미수락 종료를 준비하고 종료 수락 및 네이티브 프로세스 종료를 기다린다.
         * 인자: 없음
         * 반환값: 종료 작업 완료 Promise
         * 작성 날짜: 2026/09/17
         */
        ...
        // 공유 Promise를 먼저 등록해 동기 재진입도 같은 종료 작업을 기다리게 한다.
    ...
    /**
     * 함수 이름: set_shutdown_progress_listener()
     * 기능: 종료 준비 단계의 변경을 받을 callback을 등록하거나 해제한다.
     * 인자: listener -> 단계 callback 또는 해제용 null
     * 반환값: 없음
     * 작성 날짜: 2026/09/17
     */
     ...
     * 인자: liquidation_confirmed -> 종료 준비 요청에 전달할 청산 확인 여부
    ...
    private async request_json<Data>(
        ...
        /**
         * 함수 이름: abort_from_caller()
         * 기능: 호출자의 취소를 adapter 소유 요청 취소 신호에 전달한다.
         * 인자: 없음
         * 반환값: 없음
         * 작성 날짜: 2026/09/17
         */
    ...
    private async full_resynchronize(reason: string): Promise<void> {
        // 폐기·중복 조회·종료 수명에서는 새 snapshot 복구를 시작하지 않는다.
        ...
        // 취소 가능한 snapshot 조회를 시작하고 현재 세대의 결과만 재연결에 사용한다.
    ...
    /**
     * 함수 이름: clear_recovery_work()
     * 기능: 현재 세대의 재시도·socket 타이머와 resync 읽기를 정리하고 명령 멱등 식별자는 보존한다.
     * 인자: 없음
     * 반환값: 없음
     * 작성 날짜: 2026/09/17
     */
    ...
    /**
     * 함수 이름: handle_connection_failure()
     * 기능: 연결 오류를 기록하고 재시도 가능성과 현재 수명에 따라 복구를 예약하거나 종료한다.
     * 인자: error -> 정규화한 오류, stage -> 실패 단계, original -> 원본 오류
     * 반환값: 없음
     * 작성 날짜: 2026/09/17
     */
    private handle_connection_failure(error: BackendAdapterError, stage: ConnectionDiagnosticStage, original: unknown = error): void {
        // 오류의 재시도 가능성과 종료 수명을 확인한다.
        ...
        // 실패한 연결을 정리하고 다음 복구 시도에 새 세대를 부여한다.
        this.clear_recovery_work();
        ++this.#connection_generation;
    ...
    /**
     * 함수 이름: publish_recovery_status()
     * 기능: 복구 안내 발행 실패를 기록하면서 이미 예약한 snapshot 복구를 유지한다.
     * 인자: reason -> 복구 사유 코드, next_retry_at_ms -> 다음 재시도 시각 또는 null
     * 반환값: 없음
     * 작성 날짜: 2026/09/17
     */
    ...
    /**
     * 함수 이름: arm_socket_timeout()
     * 기능: 현재 socket과 세대에만 유효한 연결·수신 시간 초과 검사를 예약한다.
     * 인자: delay -> 대기 시간(ms), code -> 오류 코드, generation -> 연결 세대, socket -> 검사 대상
     * 반환값: 없음
     * 작성 날짜: 2026/09/17
     */
    ...
    /**
     * 함수 이름: mark_event_stream_ready()
     * 기능: 정상 수신 시각과 연결 상태를 갱신하고 복구 사건·heartbeat·발행 회복 예산을 관리한다.
     * 인자: 없음
     * 반환값: 없음
     * 작성 날짜: 2026/09/17
     */
    ...
    /**
     * 함수 이름: event_stream_is_stale()
     * 기능: 평소에는 단조 시각으로 수신 지연을 검사하고 OS 복귀 때만 벽시계 공백도 반영한다.
     * 인자: resumed_from_sleep -> OS 복귀 알림에서 호출했는지 여부
     * 반환값: 마지막 수신 이후 지연이 상한 이상이면 true
     * 작성 날짜: 2026/09/17
     */
    ...
    /**
     * 함수 이름: install_environment_listeners()
     * 기능: 네트워크·화면 가시성·OS 복귀 알림을 한 번 등록하고 해제 함수를 보관한다.
     * 인자: 없음
     * 반환값: 없음
     * 작성 날짜: 2026/09/17
     */
    private install_environment_listeners(): void {
        ...
        /**
         * 함수 이름: observe()
         * 기능: 환경 변화를 기록하고 기존 수신이 오래된 경우 읽기 전용 복구를 시작한다.
         * 인자: event -> 네트워크·가시성·OS 복귀 이벤트
         * 반환값: 없음
         * 작성 날짜: 2026/09/17
         */
        ...
        // 온라인·오프라인·가시성·절전 복귀를 같은 관찰 경로로 연결한다.
        ...
        // 수명 종료 시 등록한 listener를 같은 참조로 제거한다.
        this.#remove_environment_listeners = () => {
    ...
    /**
     * 함수 이름: publish_connection_status()
     * 기능: 재시도·최초 오류·종료 준비 정보를 합쳐 현재 연결 상태를 발행한다.
     * 인자: phase -> 연결 단계, next_retry_at_ms -> 다음 재시도 시각 또는 null
     * 반환값: 없음
     * 작성 날짜: 2026/09/17
     */
    ...
    /**
     * 함수 이름: error_type()
     * 기능: 진단에 원문 오류 대신 허용된 오류 종류만 남기도록 분류한다.
     * 인자: error -> 분류할 오류
     * 반환값: 진단 계약에 정의된 오류 종류
     * 작성 날짜: 2026/09/17
     */
    ...
    /**
     * 함수 이름: record_failure()
     * 기능: 명령 거부와 연결 장애를 구분하고 사건별 최초 원인을 보존하여 기록한다.
     * 인자: error -> 오류, stage -> 실패 단계, details -> 추가 고정 진단 필드
     * 반환값: 없음
     * 작성 날짜: 2026/09/17
     */
    ...
    /**
     * 함수 이름: record_connection()
     * 기능: 연결 세대·수신 위치·사건 식별자를 진단에 붙이고 저장 실패의 전파를 막는다.
     * 인자: event -> 진단 종류, details -> 추가 고정 진단 필드
     * 반환값: 없음
     * 작성 날짜: 2026/09/17
     */
    ...
    /**
     * 함수 이름: is_disposed()
     * 기능: 세션 인증 정보가 폐기되어 adapter를 더 사용할 수 없는지 읽는다.
     * 인자: 없음
     * 반환값: 세션 인증 정보가 제거되었으면 true
     * 작성 날짜: 2026/09/17
     */
    ...
    private close_current_socket(reason: string): void {
        // 현재 socket 참조를 먼저 제거해 종료 callback의 재진입을 분리한다.
        ...
        // 이벤트 handler를 해제한 뒤 socket을 닫고 종료 오류는 진단으로 남긴다.
    ...
    private normalize_adapter_failure(error: unknown): BackendAdapterError {
        // 알려진 오류는 코드와 재시도 가능성을 보존해 adapter 오류로 변환한다.
        ...
        // 분류할 수 없는 수신 실패에는 고정된 재시도 가능 오류를 사용한다.
    ...
    private fail_closed(error: BackendAdapterError): void {
        ...
        // 인증 참조와 callback을 분리한 뒤 마지막 실패 통지를 전달한다.
