...
/**
 * 클래스 이름: Socket
 * 기능: 외부 네트워크 없이 WebSocket의 연결·수신·단절 순서를 제어하는 테스트 대역이다.
 * 작성 날짜: 2026/09/17
 */
class Socket implements BackendWebSocket {
    ...
    /**
     * 함수 이름: receive()
     * 기능: 지정 순번의 계좌 이벤트를 현재 메시지 callback에 전달한다.
     * 인자: sequence -> 전달할 backend 이벤트 순번
     * 반환값: 없음
     * 작성 날짜: 2026/09/17
     */
    ...
    /**
     * 함수 이름: open()
     * 기능: socket 연결 완료를 현재 open callback에 알린다.
     * 인자: 없음
     * 반환값: 없음
     * 작성 날짜: 2026/09/17
     */
...
/**
 * 함수 이름: response()
 * 기능: 요청 식별자와 fixture payload를 성공 응답 envelope로 감싼다.
 * 인자: init -> 요청 옵션, data -> 응답 데이터, status -> HTTP 상태
 * 반환값: 검증 가능한 Response
 * 작성 날짜: 2026/09/17
 */
...
/**
 * 함수 이름: setup()
 * 기능: 가짜 socket·HTTP·진단 기록으로 연결 복구 검증 환경을 시작한다.
 * 인자: fetcher -> 사용할 HTTP 테스트 구현
 * 반환값: adapter·socket 목록·callback·진단 기록
 * 작성 날짜: 2026/09/17
 */
...
describe('long-running backend connection recovery', () => {
    ...
    it('transient snapshot failures preserve authentication and retry until a verified event resumes', async () => {
        ...
        expect(adapter.is_disposed).toBe(false);  // 반환값과 관찰한 상태가 시나리오의 기대값과 일치하는지 검증한다.
    ...
    it('does not wait forever for open or for a silent event stream', async () => {
        ...
        expect(logs.some((record) => record.error_code === 'EVENT_STREAM_CONNECT_TIMEOUT')).toBe(true);  // 반환값과 관찰한 상태가 시나리오의 기대값과 일치하는지 검증한다.
