...
describe('NativeSidecarLifecycle Phase 12 contract', () => {
    it.each([
        ...
        expect(validate_native_sidecar_exit_payload(payload)).toEqual(payload);
    });
    ...
    it.each([
        ...
        ({ payload, expected_result }) => {
            expect(is_expected_normal_sidecar_exit(payload)).toBe(expected_result);
        },
    );
    ...
    it.each([
        ...
        expect(validate_native_exit_request_payload(payload)).toEqual(payload);
    });
