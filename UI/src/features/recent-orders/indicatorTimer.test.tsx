...
describe('실시간 지표 타이머', () => {
    ...
    it('서버 측 오래된 평가와 수신 후 시간을 보정하며 PC 시각 변경에 영향받지 않는다', () => {
        ...
        expect(present_indicator_timer(model, 2000).time).toBe('02:29');
    ...
    it('연속 조건 중단 시 전체 시간에서 멈추고 새 회차는 처음부터 계산한다', () => {
        ...
        act(() => vi.advanceTimersByTime(1000));
        expect(screen.getByRole('timer')).toHaveTextContent('02:29');
        ...
        expect(screen.getByText('정지 · 조건 미충족')).toBeInTheDocument();
    ...
    it('동일 데이터 재전송은 최초 수신 시각을 유지하고 새로운 서버 측정만 보정한다', () => {
        ...
        expect(screen.getByRole('timer')).toHaveTextContent('02:50');
