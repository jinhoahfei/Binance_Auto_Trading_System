...
> **2026-09-22 수수료 정책 갱신:** 신규 현물 거래는 BNB 납부 없이 매수·매도 각각
> 0.1%를 적용하며 할인 납부 미적용과 양방향 비할인 요율을 검증한다. BNB 잔액·가격은
> 신규 거래 전제조건에서 제외한다. 현재 기준은 [현물 수수료 정책](SPOT_FEE_POLICY.md)이다.
> 아래 BNB 지원·잔액 준비·패키지 PASS와 수수료 차단에 관한 기록은 당시 구현·검증 이력이다.
> 과거 실제 BNB 체결 회계와 복구 호환은 보존하며 기존 이력의 수수료는 재계산하지 않는다.
